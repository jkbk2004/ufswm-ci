...
export PATH="/glade/work/epicufsrt/contrib/hpc-stack/bin/bison/bin:$PATH"

echo "bison is `which bison`, version `bison -V` "

cmake VERBOSE=1 -DCMAKE_C_FLAGS="-std=gnu99" -DCMAKE_Fortran_COMPILER=${FC} -DCMAKE_C_COMPILER=${CC} -DMAKE_CXX_COMPILER=${CXX}   -DCMAKE_INSTALL_PREFIX=$prefix -DCMAKE_BUILD_TYPE=Release -DTHREADS=OFF -DMPI_THREAD_MULTIPLE=OFF ..
